...
public:
	...
	add(Expression* first, Expression* second) :e1(first), e2(second) { }
...
public:
	...
	ifexpr(Expression* expr1, Expression* expr2, Expression* expr_then, Expression* expr_else):e1(expr1), e2(expr2), e_then(expr_then), e_else(expr_else){}
	~ifexpr(){ delete e1; delete e2; delete e_then; delete e_else; }
};
...
public:
	Expression* eval(std::unordered_map<std::string, Expression*> env) {
		...
		if (pr.second == false) (pr.first)->second = value;

		return body->eval(env);
	}

	let(std::string i, Expression* val, Expression* b): body(b), value(val), id(i){ }
	~let() { delete body; delete value; }
...
class function : public Expression {
	...
	//friend class call;
public:
	function(std::string i, Expression* expression) : id(i), body(expression) {
		/*auto it = env.find(id);
		if (it == env.end()) {
			return;
		}
...
class call : public Expression {
	...
	Expression* arg_expr;
public:
	call(Expression* fExpression, Expression* argExpression) : f_expr(fExpression), arg_expr(argExpression) {}
	~call() { delete f_expr; delete arg_expr; }
	...
	Expression* eval(std::unordered_map<std::string, Expression*> env) {
		...
		if (pr.second == false) (pr.first)->second = arg_expr->eval(env);
